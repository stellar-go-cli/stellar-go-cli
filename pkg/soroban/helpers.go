...
// mustScVal unwraps an ScVal constructor result. NewScVal only errors on a
// type/value mismatch, which these fixed-type helpers cannot produce.
func mustScVal(v xdr.ScVal, err error) xdr.ScVal {
	if err != nil {
		panic(fmt.Sprintf("invalid ScVal: %v", err))
	}
	return v
}

func ScvString(s string) xdr.ScVal {
	return mustScVal(xdr.NewScVal(xdr.ScValTypeScvString, xdr.ScString(s)))
}

func ScvSymbol(s string) xdr.ScVal {
	return mustScVal(xdr.NewScVal(xdr.ScValTypeScvSymbol, xdr.ScSymbol(s)))
}

func ScvBytes(b []byte) xdr.ScVal {
	return mustScVal(xdr.NewScVal(xdr.ScValTypeScvBytes, xdr.ScBytes(b)))
}

func ScvBool(b bool) xdr.ScVal {
	return mustScVal(xdr.NewScVal(xdr.ScValTypeScvBool, b))
}

func ScvU32(n uint32) xdr.ScVal {
	return mustScVal(xdr.NewScVal(xdr.ScValTypeScvU32, xdr.Uint32(n)))
}

func ScvU64(n uint64) xdr.ScVal {
	return mustScVal(xdr.NewScVal(xdr.ScValTypeScvU64, xdr.Uint64(n)))
}

func ScvI32(n int32) xdr.ScVal {
	return mustScVal(xdr.NewScVal(xdr.ScValTypeScvI32, xdr.Int32(n)))
}

func ScvI64(n int64) xdr.ScVal {
	return mustScVal(xdr.NewScVal(xdr.ScValTypeScvI64, xdr.Int64(n)))
}

func ScvAddress(addr xdr.ScAddress) xdr.ScVal {
	return mustScVal(xdr.NewScVal(xdr.ScValTypeScvAddress, addr))
}
